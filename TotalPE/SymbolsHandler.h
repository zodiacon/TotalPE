...
public:
    SymbolInfo();
    SymbolInfo(SymbolInfo const&) = delete;
    SymbolInfo& operator=(SymbolInfo const&) = delete;
...
public:
    SymbolsHandler(HANDLE hProcess = ::GetCurrentProcess(), PCSTR searchPath = nullptr, DWORD symOptions =
        SYMOPT_UNDNAME | SYMOPT_CASE_INSENSITIVE | SYMOPT_AUTO_PUBLICS | SYMOPT_INCLUDE_32BIT_MODULES | SYMOPT_DEFERRED_LOADS);
